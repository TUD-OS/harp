...
MockMessageListener::MockMessageListener(const std::string &socket_path, MockClient *client) :
...
void MockMessageListener::add_subscriber(const FeatureID &feature_id, Feature *feature)
{
    _subscribers.emplace(feature_id, feature);
}

ClientResponse MockMessageListener::forward(const MockServerMessage &msg) const
{
    return _client->handle(msg);
}

void *MockMessageListener::listening(void *args)
{
    auto push_server = reinterpret_cast<MockMessageListener *>(args);
...
MockMessageListener::~MockMessageListener()
{
