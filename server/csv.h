...
class Iter {
 private:
  ...
  Iter(Outer* outer, iterator i) : _outer{outer}, _i{i} {}
 ...
 public:
  ...
  value_type operator*() { return _outer->get(*_i); }
...
class ConstIter {
 private:
  ...
  ConstIter(const Outer* outer, iterator i) : _outer{outer}, _i{i} {}
 ...
 public:
  ...
  bool operator==(const ConstIter& o) const { return _i == o._i; }
...
class CSVData {
 ...
 public:
  ...
  CSVData(const std::string& file, char sep = ',', bool column_names = true,
          bool row_names = true)
  ...
  void from_file(const std::string& file, char sep = ',',
    ...
    std::ifstream f{file};
    if (!f.is_open()) throw std::runtime_error{"Can't open file " + file + "."};
  ...
  void to_file(const std::string& file, char sep = ',',
    ...
    for (auto& row : _rows) {
      ...
      if (row_names) elements.push_back(row);

      for (auto& col : _columns) elements.push_back(_data[row][col]);
  ...
  std::vector<std::string> rows() const { return _rows; }
 ...
 public:
  class RowIter {
   ...
   private:
    ...
    RowIter(CSVData* data) : _data{data}, _elements{data->rows()} {}
   ...
   public:
    iterator begin() { return iterator{this, _elements.begin()}; }

    const_iterator begin() const {
      return const_iterator{this, _elements.begin()};
    }

    iterator end() { return iterator{this, _elements.end()}; }

    const_iterator end() const { return const_iterator{this, _elements.end()}; }
  ...
  class ConstRowIter {
   ...
   private:
    ...
    ConstRowIter(const CSVData* data) : _data{data}, _elements{data->rows()} {}
  ...
  class ColumnIter {
   ...
   private:
    ...
    ColumnIter(CSVData* data) : _data{data}, _elements{data->columns()} {}
 ...
 public:
  ...
  row_iterator row_iter() { return row_iterator{this}; }

  const_row_iterator row_iter() const { return const_row_iterator{this}; }

  const_row_iterator const_row_iter() const { return const_row_iterator{this}; }

  column_iterator column_iter() { return column_iterator{this}; }
...
class Slice {
 ...
 private:
  reference get(const std::string& name) { return at(name); }

  value_type get(const std::string& name) const { return at(name); }
 ...
 public:
  ...
  std::string fixed() const { return _fixed; }

  std::vector<std::string> names() const { return _other_dim; }
  ...
  reference operator()(const std::string& name) { return at(name); }

  value_type operator()(const std::string& name) const { return at(name); }

  reference operator()(int nr) { return at(nr); }

  value_type operator()(int nr) const { return at(nr); }

  iterator begin() { return iterator{this, _other_dim.begin()}; }

  const_iterator begin() const {
    return const_iterator{this, _other_dim.begin()};
  }

  iterator end() { return iterator{this, _other_dim.end()}; }

  const_iterator end() const { return const_iterator{this, _other_dim.end()}; }

  const_iterator cbegin() const {
    return const_iterator{this, _other_dim.begin()};
  }

  const_iterator cend() const { return const_iterator{this, _other_dim.end()}; }
};
...
CSVData::RowIter::value_type CSVData::RowIter::get(
    const std::string& name) const {
  return const_cast<const CSVData*>(_data)->row(name);
}

CSVData::ConstRowIter::value_type CSVData::ConstRowIter::get(
    const std::string& name) const {
  return _data->row(name);
}

CSVData::ColumnIter::reference CSVData::ColumnIter::get(
    const std::string& name) {
  return _data->column(name);
}

CSVData::ColumnIter::value_type CSVData::ColumnIter::get(
    const std::string& name) const {
  return const_cast<const CSVData*>(_data)->column(name);
}

CSVData::ConstColumnIter::value_type CSVData::ConstColumnIter::get(
    const std::string& name) const {
  return _data->column(name);
}
