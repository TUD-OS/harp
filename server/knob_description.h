...
// TODO: Consider removing the following paraemeters from the KnobDescription:
// max_replica, allowed cores for the processes etc.
class KnobDescription {
 public:
  ...
  struct RegionBodySpecification {
    ...
    /// \brief Maximum number of replicas. If this field equals zero then there
    /// are no maximum number of replicas.
  ...
  /**
   * \brief Specify if a characteristic is better if the value is lower or the
   * opposite.
   */
  enum class CharacteristicComparison { LessIsBetter, MoreIsBetter };

  /// \brief Defines specifications for characteristics
  using CharacteristicSpecifications =
      std::map<std::string, CharacteristicComparison>;
  ...
  bool is_mapping_valid(const Mapping &);

 private:
  bool valid;

  bool check_validity_process(const ProcessSpecifications &,
                              const ProcessAffinities<int> &);
};
